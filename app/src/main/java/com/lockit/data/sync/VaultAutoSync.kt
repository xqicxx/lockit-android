...
class VaultAutoSync(
    ...
    fun onCredentialChanged() {
        scope.launch {
            ...
            if (syncManager.hasSyncKey()) {
                syncManager.push(ResolveStrategy.LastWriteWins)
    ...
    private suspend fun uploadCloudBackup() {
        try {
            val key = syncManager.getSyncKeyBytes() ?: return
            val dbFile = syncManager.getDatabaseFileRef()
